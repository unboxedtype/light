...
type Instruction =
    ...
    // Handful operations introduced for optimization
    | Inc | Dec | IsZero | IsOne
and
...
let unaryop (op : int -> int) state =
    let heap = getHeap state
    let (a1 :: s') = getStack state
    let (NNum n1) = heapLookup heap a1
    let (heap', a) = heapAlloc heap (NNum (op n1))
    putStack (a :: s') (putHeap heap' state)
...
let inc state =
    unaryop (fun a -> a + 1) state

let dec state =
    unaryop (fun a -> a - 1) state

let iszero state =
    unaryop (fun a -> if (a = 0) then True else False) state

let isone state =
    unaryop (fun a -> if (a = 1) then True else False) state
...
let rec compileE (ast : Expr) (env: GmEnvironment) : GmCode =
    match ast with
        ...
        | EDec e ->
            (compileE e env) @ [Dec]
        | EIsZero e ->
            (compileE e env) @ [IsZero]
        | EIsOne e ->
            (compileE e env) @ [IsOne]
