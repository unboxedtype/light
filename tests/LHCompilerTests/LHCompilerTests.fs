...
let testLetTuple1 () =
    let prog = "contract testTuple5
                type MessageBody = { seqNo: uint32; nextFun: int -> int }
                type Message = { src:ActorId; dst:ActorId; body: MessageBody }
                (* For this type, we have autogenerated function: *)
                (* readMsg: Cell -> Message *)
                type State = { n: int }
                let main msgCell (st:State) =
                    let msg = readMsg msgCell in
                    (* execute the received message and save the
                       result into the state *)
                    { n = msg.body.nextFun n }
                "
    execAndCheckPrint prog false false "[ 0 1 2 6 ]"
